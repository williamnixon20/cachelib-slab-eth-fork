...
class MMTinyLFUTail {
 public:
  ...
  enum LruType { Main, MainTail, Tiny, NumTypes };
  ...
  struct Container {
   ...
   public:
    ...
    LruType getLruType(const T& node) noexcept {
      return isTiny(node) ? LruType::Tiny : (inTail(node) ? LruType::MainTail : LruType::Main);
    }
   private:
    ...
    bool inTail(const T& node) const noexcept {
      return config_.tailSize && node.template isFlagSet<RefFlags::kMMFlag2>();
    }
    // for tails
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
MMTinyLFUTail::Container<T, HookPtr>::Container(
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T,
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
bool MMTinyLFUTail::Container<T, HookPtr>::recordAccess(T& node,
  ...
  if (node.isInMMContainer() &&
    ...
    if (inTail(node)) {
        unmarkTail(node);
        lru_.getList(LruType::MainTail).remove(node);
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
cachelib::EvictionAgeStat MMTinyLFUTail::Container<T, HookPtr>::getEvictionAgeStat(
    uint64_t projectedLength) const noexcept {
  LockHolder l(lruMutex_);
...
MMTinyLFUTail::Container<T, HookPtr>::getEvictionAgeStatLocked(
    uint64_t projectedLength) const noexcept {
  ...
  stat.warmQueueStat.oldestElementAge =
      getOldestAgeLocked(LruType::MainTail, currTime);
  stat.warmQueueStat.size = lru_.getList(LruType::Main).size() +
                            lru_.getList(LruType::MainTail).size();
  stat.warmQueueStat.projectedAge =
      getProjectedAge(LruType::MainTail, stat.warmQueueStat.oldestElementAge);

  return stat;
}

template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::updateFrequenciesLocked(
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::maybePromoteTailLocked() noexcept {
  // Choose eviction candidate and place it at the tail of tiny cache
  // from where evictions occur.
  auto mainNode = lru_.getList(LruType::MainTail).getTail();
  if (!mainNode) {
    mainNode = lru_.getList(LruType::Main).getTail();
    if (!mainNode) {
      return;
    }
  }
  ...
  if (admitToMain(*tinyNode, *mainNode)) {
    ...
    if (inTail(*mainNode)) {
      unmarkTail(*mainNode);
      lru_.getList(LruType::MainTail).remove(*mainNode);
    } else {
  ...
  if (inTail(*mainNode)) {
    unmarkTail(*mainNode);
    lru_.getList(LruType::MainTail).remove(*mainNode);
    lru_.getList(LruType::Main).linkAtHead(*mainNode);
  } else {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
bool MMTinyLFUTail::Container<T, HookPtr>::add(T& node) noexcept {
  const auto currTime = static_cast<Time>(util::getCurrentTimeSec());
  LockHolder l(lruMutex_);
  if (node.isInMMContainer()) {
    return false;
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
typename MMTinyLFUTail::Container<T, HookPtr>::LockedIterator
MMTinyLFUTail::Container<T, HookPtr>::getEvictionIterator() const noexcept {
  LockHolder l(lruMutex_);
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
template <typename F>
void MMTinyLFUTail::Container<T, HookPtr>::withEvictionIterator(F&& fun) {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
template <typename F>
void MMTinyLFUTail::Container<T, HookPtr>::withContainerLock(F&& fun) {
  LockHolder l(lruMutex_);
  fun();
}

template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::removeLocked(T& node, bool doRebalance) noexcept {
  if (isTiny(node)) {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
bool MMTinyLFUTail::Container<T, HookPtr>::remove(T& node) noexcept {
  LockHolder l(lruMutex_);
  if (!node.isInMMContainer()) {
    return false;
  }
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::remove(LockedIterator& it) noexcept {
  T& node = *it;
  XDCHECK(node.isInMMContainer());
  ++it;
  removeLocked(node, false);
}

template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
bool MMTinyLFUTail::Container<T, HookPtr>::replace(T& oldNode,
                                               T& newNode) noexcept {
  LockHolder l(lruMutex_);
  ...
  if (isTiny(oldNode)) {
    ...
    unmarkTiny(oldNode);
    markTiny(newNode);
  } else if (inTail(oldNode)) {
    lru_.getList(LruType::MainTail).replace(oldNode, newNode);
    unmarkTail(oldNode);
    markTail(newNode);
  } else {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
typename MMTinyLFUTail::Config MMTinyLFUTail::Container<T, HookPtr>::getConfig() const {
  LockHolder l(lruMutex_);
  return config_;
}

template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::setConfig(const Config& c) {
  LockHolder l(lruMutex_);
  config_ = c;
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
serialization::MMTinyLFUObject MMTinyLFUTail::Container<T, HookPtr>::saveState()
    const noexcept {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
MMContainerStat MMTinyLFUTail::Container<T, HookPtr>::getStats() const noexcept {
  LockHolder l(lruMutex_);
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::reconfigureLocked(const Time& currTime) {
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
MMTinyLFUTail::Container<T, HookPtr>::LockedIterator::LockedIterator(
    LockHolder l, const Container<T, HookPtr>& c) noexcept
    : c_(c),
      tIter_(c.lru_.getList(LruType::Tiny).rbegin()),
      mIter_(c.lru_.getList(LruType::MainTail).rbegin()),
      l_(std::move(l)) {}

template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
void MMTinyLFUTail::Container<T, HookPtr>::adjustTail() {
  auto list = LruType::Main;
  auto tailList = LruType::MainTail;
...
template <typename T, MMTinyLFUTail::Hook<T> T::*HookPtr>
uint32_t MMTinyLFUTail::Container<T, HookPtr>::getOldestAgeLocked(
