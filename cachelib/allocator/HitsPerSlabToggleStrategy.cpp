...
HitsPerSlabToggleStrategy::HitsPerSlabToggleStrategy(Config config)
    : RebalanceStrategy(HitsPerSlabToggle), config_(std::move(config)) {}
...
ClassId HitsPerSlabToggleStrategy::pickVictim(const Config& config,
  ...
  return *std::min_element(
      victims.begin(), victims.end(), [&](ClassId a, ClassId b) {
        ...
        return poolState.at(a).projectedDeltaHitsTogglePerSlab(stats) *
                   weight_a <
               poolState.at(b).projectedDeltaHitsTogglePerSlab(stats) *
                   weight_b;
      });
}
...
ClassId HitsPerSlabToggleStrategy::pickReceiver(const Config& config,
                                                PoolId pid,
                                                const PoolStats& stats,
  ...
  return *std::max_element(
      receivers.begin(), receivers.end(), [&](ClassId a, ClassId b) {
        ...
        double weight_b =
            config.getWeight ? config.getWeight(pid, b, stats) : 1;
        return poolState.at(a).deltaHitsTogglePerSlab(stats) * weight_a <
               poolState.at(b).deltaHitsTogglePerSlab(stats) * weight_b;
      });
}

RebalanceContext HitsPerSlabToggleStrategy::pickVictimAndReceiverImpl(
  ...
  const auto victimProjectedDeltaHitsPerSlab =
      poolState.at(ctx.victimClassId).projectedDeltaHitsTogglePerSlab(poolStats) *
      weightVictim;
  const auto receiverDeltaHitsPerSlab =
      poolState.at(ctx.receiverClassId).deltaHitsTogglePerSlab(poolStats) *
      weightReceiver;
  ...
  if (receiverDeltaHitsPerSlab < victimProjectedDeltaHitsPerSlab ||
    ...
    XLOG(DBG, " Not enough to trigger slab rebalancing, diffRatio: {}",
         config.diffRatio);
    return kNoOpContext;
  }
  ...
  for (const auto i : poolStats.getClassIds()) {
    poolState[i].updateHitsToggle(poolStats);
  }

  return ctx;
}

ClassId HitsPerSlabToggleStrategy::pickVictimImpl(const CacheBase& cache,
                                            PoolId pid,
