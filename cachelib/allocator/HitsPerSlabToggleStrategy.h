...
// with the lowest hits that TOGGLE STATUS FROM UNVISITED TO VISITED
// per slab to the highest hits per slab within the pool.
class HitsPerSlabToggleStrategy : public RebalanceStrategy {
 public:
  ...
  explicit HitsPerSlabToggleStrategy(Config config = {});
