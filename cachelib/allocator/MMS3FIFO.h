 ...
 public:
  ...
  struct Container {
   ...
   public:
    ...
    class LockedIterator {
     public:
      ...
      T* firstCandidate_ = nullptr;
      bool fromProb_ = false;
      // Once we run out of iterator items, we can evict accessed items too.
      bool returnAccessed_ = false;
      ...
      LockedIterator(FIFOList* qdlist, LockHolder l)
          : qdlist_(qdlist), l_(std::move(l)) {
        auto ref = qdlist_->getEvictionCandidateRef();
        candidate_ = ref.node;
        fromProb_ = ref.fromProb;
        firstCandidate_ = candidate_;
      }
    ...
    template <typename F>
    void withEvictionIterator(F&& fun) {
   ...
   private:
    ...
    // Bit MM_BIT_3 is used to record if the item is at the tail of the fifo.
    void markTail(T& node) noexcept {
      node.template setFlag<RefFlags::kMMFlag3>();
    }
    void unmarkTail(T& node) noexcept {
      node.template unSetFlag<RefFlags::kMMFlag3>();
    }
    bool isTail(const T& node) const noexcept {
      return node.template isFlagSet<RefFlags::kMMFlag3>();
    }

    uint32_t numTail_{0};
    T* lastTail_ = nullptr;

    // Naive way of marking tail, get the last element of main list
    // and iterate all the way back.
    // TODO: Keep track of last tail and counter, only add necessary delta tail
    // from target.
    void rebalanceTail() noexcept {
      auto* pfifo_ = &qdlist_.getListProbationary();
      auto* mfifo_ = &qdlist_.getListMain();
      ...
      T* node = mfifo_->getTail();
      size_t count = 0;

      try {
        while (node != nullptr && node->isInMMContainer() && count < nTail) {
          if (count < nTail) {
            if (!isTail(*node)) {
              markTail(*node);
              numTail_++;
          ...
          count++;
          node = mfifo_->getPrev(*node);
        }
      } catch (const std::exception& e) {
        printf("[RebalanceTail] Unknown exception occurred (at count=%zu)\n",
               count);
        printf("Exception: %s\n", e.what());
      }
      ...
      // // Log what we did
      // printf(
      //     "RebalanceTail: listSize=%zu, targetTail=%zu, counted=%zu, "
      //     "numTail_=%u\n",
      //     listSize, nTail, count, numTail_);
    }

    void rebalanceTail2() noexcept {
      auto* mfifo_ = &qdlist_.getListMain();

      // Combine total list size and desired tail size
      size_t listSize = mfifo_->size();
      // max is listSize * tailSize
      size_t nTail = std::min(config_.tailSize, listSize);

      if (numTail_ >= nTail) {
        return;
      }

      // compute how many new nodes to mark
      size_t needed = nTail - numTail_;

      // ensure lastTail_ points to a valid node in main fifo
      if (!lastTail_ || !lastTail_->isInMMContainer()) {
        lastTail_ = mfifo_->getTail();
      }

      // choose starting node
      T* node = nullptr;
      if (lastTail_ && lastTail_ != mfifo_->getTail()) {
        node = mfifo_->getPrev(*lastTail_);
      } else {
        node = mfifo_->getTail();
      }
      size_t count = 0;
      try {
        while (node != nullptr && node->isInMMContainer() && count < needed) {
          if (!isTail(*node)) {
            markTail(*node);
            numTail_++;
            count++;
        ...
        printf("[RebalanceTail] Exception at count=%zu: %s\n", count, e.what());
      }
